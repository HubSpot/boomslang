...
Boomslang runs CPython 3.15.0a7 from a WASI build. The default artifact embeds that runtime in Java through [Endive](https://github.com/bytecodealliance/endive), so Python runs inside the JVM without JNI, subprocesses, or a system Python install — sandboxed, with the stdlib plus NumPy, Pandas, Matplotlib, Pillow, Pydantic, ijson, and Jinja2 included. The same runtime is also packaged as a Python wheel (`boomslang-py/`, executed with wasmtime) and embeddable from Rust or any WASM runtime via a language-neutral ABI.
