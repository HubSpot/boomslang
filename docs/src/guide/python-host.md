...
Run sandboxed Python *from Python*. The `boomslang-py/` package bundles the same WASM runtime as the Java artifact — CPython 3.14 with NumPy, Pandas, Matplotlib, Pillow, Pydantic, ijson, openpyxl, and xlrd — and executes it with [wasmtime](https://pypi.org/project/wasmtime/). Guest code has no network access and can only touch the directories you mount.
