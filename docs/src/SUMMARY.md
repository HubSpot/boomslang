# Summary

[Introduction](introduction.md)

# Getting Started

- [Quickstart](quickstart.md)
- [Installation & Runtime Variants](installation.md)

# User Guide

- [Running Python from Java](guide/running-python.md)
- [Lifecycle, Timeouts & Limits](guide/lifecycle.md)
- [Calling Host Functions from Python](guide/host-functions.md)
- [Async Host Calls](guide/async.md)
- [Adding Python Modules & the Overlay](guide/python-modules.md)
- [Supported Python Libraries](guide/supported-libraries.md)
- [Custom Python Builds](guide/custom-python-builds.md)
- [Embedding from Rust (and Other Hosts)](guide/other-hosts.md)

# Reference

- [Base ABI Specification](reference/abi.md)
- [Extension ABI JSON & Lowering](reference/extension-abi.md)
- [Async Wire Protocol](reference/async-protocol.md)
- [hostgen DSL Reference](reference/hostgen-dsl.md)
- [Glossary](reference/glossary.md)
